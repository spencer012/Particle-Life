...
static const int window_w = 1920;//1600;
static const int window_h = 1080;//900;
static const int steps_per_frame_normal = 5;

void chaos(Universe &universe) {
	universe.SetPopulation(6, 350);
...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  //Create the universe of particles
	...
	Universe universe(1, 1, window_w, window_h);
	chaos(universe);
	...
	while (window.isOpen()) {
		...
		*/
		...
		if (universe.lowActivity) {
			universe.lowActivity = false;
			if (startDelay <= 0) {
				lowActivityTimes++;

				if (lowActivityTimes > 10) {
					lowActivityTimes = 0;
					startDelay = 100;
					chaos(universe);
				}
